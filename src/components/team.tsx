...
export default function Team() {
  ...
  useGSAP(
    () => {
      const tiles = gsap.utils.toArray<HTMLElement>(".team-member-tile", stageRef.current);
      if (reducedMotion) return;
      gsap.fromTo(
        tiles,
        { opacity: 0, y: 36, rotate: (index) => (index % 2 ? 3 : -3) },
        {
          opacity: 1,
          y: 0,
          rotate: 0,
  ...
  return (
    <section id="team" className="relative overflow-hidden border-t border-line bg-night py-10 text-paper-soft md:py-32">
      ...
      <div className="relative mx-auto max-w-[1320px] px-5 md:px-[6vw]">
        ...
        <div ref={stageRef} className="grid gap-7 lg:grid-cols-[0.72fr_1.28fr] lg:gap-[6vw]">
          <Reveal className="relative flex flex-col justify-between border-t border-white/15 pt-6">
            ...
            <div className="mt-10 lg:mt-auto">
              ...
              <p className="mt-6 max-w-[470px] text-xs leading-[2] text-[#aaa198]">{active.bio}</p>
          ...
          <div className="relative min-h-[520px] md:min-h-[600px]">
            ...
            <div className="pointer-events-none absolute left-[12%] top-[8%] h-[72%] w-[72%] rounded-full border border-gold-soft/15 [animation:spin-slow_28s_linear_infinite]" aria-hidden>
            ...
            <div className="absolute left-[7%] top-[11%] h-[68%] w-[74%] overflow-hidden rounded-2xl border bg-night p-2 shadow-[0_30px_100px_rgba(0,0,0,0.45)] transition-all duration-700 md:p-3" style={{ transform: `rotate(${activeIndex % 2 ? 1 : -1}deg)`, borderColor: `${active.accent}88` }}>
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 md:p-8">
                ...
                <p className="mt-1 text-[11px] text-[#d4cbc0]">{active.role}</p>
              </div>
            </div>
                ...
                {[...localizedMembers, ...localizedMembers].map((member, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "team-member-tile group relative mx-1 h-[78px] w-[112px] shrink-0 overflow-hidden rounded-md border transition-all duration-500 md:h-[92px] md:w-[132px]",
                        isActive ? "border-gold-soft bg-gold-soft/15 shadow-[0_0_24px_rgba(229,210,163,0.2)]" : "border-white/15 bg-night/80 hover:border-gold-soft/60",
                      )}
                    >
                      ...
                      <span className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-night/95 to-transparent px-2 pb-1.5 pt-5 text-left font-serif text-xs text-paper-soft">{member.name}</span>
